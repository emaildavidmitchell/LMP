...
class Classifier():
		...
		for cat in categories:
			...
			for filename in os.listdir(directory2):
				if filename.endswith(".txt"):
					documents.append([self.get_words(directory2,filename),self.key[filename],cat])
		self.training_set = [(self.document_features(d,a), c) for (d,a,c) in documents]
	...
	def classify_file(self,directory,filename):
		...
		features = self.document_features(document,self.key[filename])
